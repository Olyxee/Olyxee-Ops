...
const departmentHealthCurveFor=(tasks:Task[],now:number,maxHours?:number):DepartmentHealthPoint[]=>{
  ...
  const hourlyPoints=Array.from({length:hours},(_,index)=>{
    ...
    const updated=tasks.reduce((count,task)=>count+(task.updates||[]).filter(update=>{const value=time(update.createdAt);return Number.isFinite(value)&&value>=start&&value<=cutoff}).length,0);
     ...
     const completedHour=cutoff>=start+hour-1;
     const startHour=new Date(start).getHours();
     const workingHour=startHour>=8&&startHour<18;
     const inactivity=completedHour&&workingHour&&positive===0&&blocked===0&&changesRequested===0&&reopened===0&&overdue===0?1:0;
...
function DepartmentHealthPanel({departmentName,tasks}:{departmentName:string;tasks:Task[]}){
  ...
  return <section className="panel admin-department-health" aria-labelledby="admin-department-health-title">
    <div className="admin-department-health-head"><div><span className="panel-kicker">Week to date · hourly activity</span><h2 id="admin-department-health-title">Delivery health</h2><p>Each point represents one elapsed hour since Monday. Blue rises show task activity in that hour; red drops show blockers, overdue work, or an inactive working hour. Overnight hours remain neutral.</p></div><div className="admin-department-health-score"><strong>{score}</strong><span>/ 100</span><small className={`manager-detail-state ${state.toLowerCase().replace(/\s+/g,"-")}`}>{state}</small></div></div>
