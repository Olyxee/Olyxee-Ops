...
const latestTaskActivityFor=(person:User,tasks:Task[])=>{
  const personName=person.name.trim().toLowerCase();
  return Math.max(0,...tasks.flatMap(task=>[
    task.createdBy===person.id?activityTimestamp(task.createdDate):0,
    ...(task.updates||[]).map(update=>update.authorName?.trim().toLowerCase()===personName?activityTimestamp(update.createdAt):0),
    ...(task.activityLog||[]).map(activity=>activity.actorName?.trim().toLowerCase()===personName?activityTimestamp(activity.createdAt):0),
  ]));
};
const lastActivityLabel=(value?:string|number)=>{
  ...
  return`Last active ${new Date(timestamp).toLocaleDateString([],{day:"numeric",month:"short",year:new Date(timestamp).getFullYear()===new Date().getFullYear()?undefined:"numeric"})}`;
};
...
const departmentHealthCurveFor=(tasks:Task[],now:number,maxHours?:number):DepartmentHealthPoint[]=>{
  ...
  const hours=Math.max(2,Math.min(maxHours||elapsedWeekHours,elapsedWeekHours));
  ...
  const formatHour=(value:number)=>new Date(value).toLocaleDateString([],{weekday:"short",hour:"numeric"});
  const hasRecentWork=tasks.some(task=>{
     const activityTimes=[
       createdAt(task),
       time(task.submittedAt),
       time(task.completedAt),
       ...(task.updates||[]).map(update=>time(update.createdAt)),
       ...(task.activityLog||[]).map(activity=>time(activity.createdAt)),
     ];
  ...
  const hourlyPoints=Array.from({length:hours},(_,index)=>{
    ...
    const submitted=tasks.filter(task=>{const value=time(task.submittedAt);return Number.isFinite(value)&&value>=start&&value<=cutoff}).length;
     const updatesInHour=tasks.flatMap(task=>(task.updates||[]).filter(update=>{const value=time(update.createdAt);return Number.isFinite(value)&&value>=start&&value<=cutoff}));
     const feedback=updatesInHour.filter(update=>["Review feedback","General comment","Manager feedback"].includes(update.type)).length;
     const updated=updatesInHour.length-feedback;
     ...
     const otherActivity=activityInHour.filter(activity=>!["Task created","Status changed to Blocked","Status changed to Changes Requested","Checklist item reopened"].includes(activity.action)).length;
...
function WorkspaceApp(){
 ...
 const flash=(s:string)=>{setNotice(s);setTimeout(()=>setNotice(""),2600)};
  ...
  useEffect(()=>{if(!user)return;loadTasks().catch(error=>flash(error instanceof Error?error.message:"Unable to load tasks."))},[user?.id]);
   useEffect(()=>{if(!user)return;let cancelled=false;const heartbeat=()=>fetch("/api/presence/heartbeat",{method:"POST"}).then(response=>{if(response.ok&&!cancelled){const now=Date.now();setOnlineUserIds(current=>current.includes(user.id)?current:[...current,user.id]);setPresenceLastSeen(current=>({...current,[user.id]:now}))}}).catch(()=>{});heartbeat();const timer=window.setInterval(heartbeat,30_000);return()=>{cancelled=true;window.clearInterval(timer)}},[user?.id]);
   useEffect(()=>{if(!user||(!isAdmin(active)&&!isManager(active)))return;let cancelled=false;const refreshPresence=()=>fetch("/api/presence").then(async response=>{if(!response.ok)throw new Error("Presence unavailable");return response.json() as Promise<{onlineUserIds:string[];lastSeenByUser?:Record<string,number>}>}).then(result=>{if(!cancelled){setOnlineUserIds(result.onlineUserIds);setPresenceLastSeen(result.lastSeenByUser||{})}}).catch(()=>{});refreshPresence();const timer=window.setInterval(refreshPresence,30_000);return()=>{cancelled=true;window.clearInterval(timer)}},[user?.id,active.role,active.opsRole,active.accessRole]);
...
function InternNav({active,onSelect}:{active:View;onSelect:(view:View)=>void}){
  const items:[View,string,any][]=[["Home","Home",LayoutDashboard],["Projects","Projects",BriefcaseBusiness],["Feedback","Feedback",MessageSquare]];
...
function CompactDepartmentHealthChart({curve,label}:{curve:DepartmentHealthPoint[];label:string}){
  const x=(index:number)=>18+index*(564/Math.max(1,curve.length-1));
  const y=(score:number)=>136-score*1.18;
  return <div className="manager-health-chart" aria-label={label}><div className="manager-health-grid"><i/><i/><i/></div><svg viewBox="0 0 600 150" preserveAspectRatio="none" role="img">{curve.slice(1).map((item,index)=>{const previous=curve[index];return <line key={`${item.label}-${index}`} className={`department-trend-segment ${item.direction}`} x1={x(index)} y1={y(previous.score)} x2={x(index+1)} y2={y(item.score)}/>})}</svg></div>;
}
  ...
  const scopedProjects=projectsData.filter(project=>isAdmin(user)||isManager(user)||project.assigneeIds.includes(user.id)||tasks.some(task=>task.project===project.name));
   ...
   const homePeople=isManager(user)?scopedPeople.filter(person=>person.id!==user.id):scopedPeople.filter(person=>person.id!==user.id&&isManager(person)).slice(0,4);
    const scopedObjectives=objectives.filter(objective=>{
      if(objective.status==="Complete")return false;
      const owner=team.find(person=>person.id===objective.managerId);
      return Boolean(user.department&&user.department!==UNASSIGNED_DEPARTMENT&&owner?.department===user.department);
  ...
  const departmentHead=isManager(user)&&user.department&&user.department!==UNASSIGNED_DEPARTMENT;
   ...
   const departmentOpen=departmentGoalTasks.filter(task=>task.status!=="Completed").length;
    const departmentReview=departmentGoalTasks.filter(task=>task.status==="Submitted for Review");
      ...
      const internActivityCurve=useMemo(()=>departmentHealthCurveFor(tasks,healthNow,24),[tasks,healthNow]);
      const internActiveTask=tasks.find(task=>task.status==="In Progress")||tasks.find(task=>!["Completed","Cancelled"].includes(task.status));
    ...
    const owner=(id?:string)=>id?((p.team||users).find(u=>u.id===id)?.name||"Unassigned"):"Unassigned";
     const myObjectives=p.objectives.filter(o=>user.role==="Super Admin"||o.managerId===user.id);
      if(user.role==="Manager"){
        const directReports=(p.team||[]).filter(person=>employmentOf(person)==="Intern"&&person.department===user.department&&isCurrentTeamMember(person));
   ...
   if(view==="Tasks")return <><Header eyebrow="Work registry" title="Tasks" subtitle={isAdmin(user)?"The complete delivery record across Olyxee.":`Work for ${user.department}.`} action={p.can("task")&&<button className="btn primary" onClick={()=>p.onModal("task")}><Plus size={14}/> New task</button>}/><TaskList tasks={tasks} {...p}/></>;
    if(view==="Projects"){const assigned=p.projectsData.filter(project=>isAdmin(user)||isManager(user)||project.assigneeIds.includes(user.id)||mine.some(task=>task.project===project.name&&task.status!=="Cancelled"));if(employmentOf(user)==="Intern")return <InternProjectFinder projects={assigned}/>;const canSeeAll=isAdmin(user)||isManager(user);return <><Header eyebrow="Delivery map" title="Projects" subtitle={canSeeAll?"All active project workspaces and ownership.":"Projects connected to your assigned tasks."} action={isAdmin(user)&&<button className="btn primary" onClick={()=>p.onModal("project")}><Plus size={14}/> Create project</button>}/><div className="project-intro"><span><b>{assigned.length}</b> active workspaces</span><span>Open a project for delivery context, people, and resources.</span></div><div className="grid project-grid">{assigned.map((project,i)=>{const people=project.assigneeIds.map(id=>(p.team||users).find(person=>person.id===id&&isCurrentTeamMember(person))).filter(Boolean) as User[];return <button className="panel project-card" key={project.id} onClick={()=>p.onProject(project.id)}><div className="project-card-top"><div className="inline"><ProjectLogo project={project}/><div className="eyebrow">{String(i+1).padStart(2,"0")} / workspace</div></div><Status s={project.status}/></div><div className="task-title">{project.name}</div><p className="project-description">{project.description}</p><div className="project-card-people"><AvatarStack people={people}/><span>{people.length} assigned {people.length===1?"person":"people"}</span></div><div className="project-card-foot"><span>View project</span><ChevronRight size={15}/></div></button>})}{!assigned.length&&<div className="empty">{canSeeAll?"No projects are available yet.":"No projects are connected to your tasks yet."}</div>}</div></>;}
     if(view==="People"){
       ...
       {canSeeEveryone?<><div className="people-summary people-segments" aria-label="Filter people"><button className={peopleFilter==="All"?"active":""} aria-pressed={peopleFilter==="All"} onClick={()=>setPeopleFilter("All")}><span className="people-segment-icon"><Users size={17}/></span><span className="people-segment-copy"><b>Everyone</b><small>Whole team</small></span><strong>{p.databasePeople?directory.length:"—"}</strong></button><button className={peopleFilter==="Employee"?"active":""} aria-pressed={peopleFilter==="Employee"} onClick={()=>setPeopleFilter("Employee")}><span className="people-segment-icon"><BriefcaseBusiness size={17}/></span><span className="people-segment-copy"><b>Employees</b><small>Core team</small></span><strong>{p.databasePeople?employees.length:"—"}</strong></button><button className={peopleFilter==="Intern"?"active":""} aria-pressed={peopleFilter==="Intern"} onClick={()=>setPeopleFilter("Intern")}><span className="people-segment-icon"><UserRound size={17}/></span><span className="people-segment-copy"><b>Interns</b><small>Learning team</small></span><strong>{p.databasePeople?interns.length:"—"}</strong></button></div><div className="people-filter-note">{p.databasePeople?<>Showing <b>{peopleFilter==="All"?"everyone":`${peopleFilter.toLowerCase()}s`}</b> · {people.length} people from Supabase</>:"Loading people from Supabase…"}</div></>:<div className="people-summary staff-summary"><button className="active"><span>Your staff</span><strong>{directory.length}</strong></button></div>}
        {p.databasePeopleError&&canSeeEveryone&&<div className="notice staff-database-notice">{p.databasePeopleError}</div>}
          <div className="panel table-wrap"><table className="table people-table"><thead><tr><th>Person</th><th>Employment</th><th>Department</th><th>Reports to</th><th>Status</th><th></th></tr></thead><tbody>{people.map(person=>{const databasePerson="source" in person&&person.source==="Supabase";const online=p.onlineUserIds.includes(person.id);const lastActivity=Math.max(activityTimestamp(p.presenceLastSeen[person.id]),activityTimestamp(person.lastSeenAt),activityTimestamp(person.lastLoginAt),latestTaskActivityFor(person,p.allTasks));return <tr key={person.id}><td><button className="people-profile-link" onClick={()=>p.onManage(person)}><Avatar person={person}/><span><b>{person.name}</b><small>{person.email||"No email recorded"}</small></span></button></td><td>{databasePerson?(person as DatabasePerson).employmentStatus||employmentOf(person):employmentOf(person)}</td><td>{person.department}{person.departmentReviewRequired&&<><br/><span className="badge amber">Supaadmin review</span></>}</td><td>{directorySource.find(item=>item.id===person.reportsTo)?.name||(databasePerson?(person as DatabasePerson).supervisorName:"")||"—"}</td><td><span className={`people-presence ${online?"online":"offline"}`}><i/>{online?"Online":lastActivityLabel(lastActivity)}</span></td><td><button className="btn people-edit-icon" onClick={()=>p.onManage(person)} aria-label={`Edit ${person.name}`} title={`Edit ${person.name}`}><Pencil size={15}/></button></td></tr>})}</tbody></table>{!people.length&&<div className="empty"><strong>No people found</strong>Try another employment filter.</div>}</div></>;
     }
    if(view==="Departments"){
...
function DepartmentTrendChart({curve,label}:{curve:DepartmentHealthPoint[];label:string}){
  ...
  return <div className="department-trend-chart">
    <div className="department-trend-plot"><div className="manager-health-grid"><i/><i/><i/></div><svg viewBox="0 0 600 190" preserveAspectRatio="xMidYMid meet" role="img" aria-label={label}>{curve.slice(1).map((point,index)=>{const previous=curve[index];return <line key={`${point.label}-${index}`} className={`department-trend-segment ${point.direction}`} x1={x(index)} y1={y(previous.score)} x2={x(index+1)} y2={y(point.score)}/>})}</svg></div>
  </div>;
}
function DepartmentHealthPanel({departmentName,tasks}:{departmentName:string;tasks:Task[]}){
  ...
  return <section className="admin-department-health" aria-labelledby="admin-department-health-title">
...
function ManagerDepartmentDetail({user,department,tasks,objectives,team,onBack,onOpen}:{user:User;department:[string,string,string];tasks:Task[];objectives:WeeklyObjective[];team:User[];onBack:()=>void;onOpen:(id:string)=>void}){
  ...
  return <div className="manager-department-detail">
    ...
    <section className="manager-detail-chart" aria-labelledby="manager-health-detail-title"><div className="manager-detail-chart-head"><div><span className="panel-kicker">Week to date · hourly activity</span><h2 id="manager-health-detail-title">Delivery health trend</h2><p>Each point represents one elapsed hour since Monday. Blue rises come from recorded task creation, updates, feedback, submissions, completions, and delivery activity. Red drops show blockers, overdue work, or work sent back for changes.</p></div><strong>{score}<small>/ 100</small></strong></div><DepartmentTrendChart curve={curve} label={`${department[0]} delivery health`}/></section>
...
function ProjectDetail({user,project,tasks,objectives,team,onBack,onOpen,onProjectUpdated,onResourceAdded,flash}:{user:User;project:Project;tasks:Task[];objectives:WeeklyObjective[];team:User[];onBack:()=>void;onOpen:(id:string)=>void;onProjectUpdated:(project:Project)=>void;onResourceAdded:(resource:ProjectResource)=>void;flash:(message:string)=>void}){
  ...
  const assignable=(manager?team.filter(person=>employmentOf(person)==="Intern"&&person.department===user.department&&person.active!==false):team.filter(isCurrentTeamMember)).sort((a,b)=>a.name.localeCompare(b.name));
  ...
  const weeklyObjectives=objectives.filter(objective=>{
    ...
    const explicitlyLinked=objective.projectIds?.includes(project.id);
    const legacyInferred=!objective.projectIds?.length&&(departmentManagerIds.includes(objective.managerId)||text.includes(projectName));
    return objective.dueDate>=weekStartDate&&objective.dueDate<=weekEndDate&&(explicitlyLinked||legacyInferred);
  }).sort((a,b)=>objectivePriorityRank[a.priority]-objectivePriorityRank[b.priority]||a.dueDate.localeCompare(b.dueDate));
  ...
  return <div className="project-detail project-detail-clear">
    ...
    <header className="project-detail-head project-detail-hero">
      <div className="project-heading"><ProjectLogo project={project} size={64}/><div><div className="project-hero-meta"><span className="eyebrow">Project workspace · {project.id}</span><span className={`project-state ${project.status.toLowerCase()}`}><i/>{project.status}</span></div><h1 className="title">{project.name}</h1><p className="subtitle">{project.description}</p><div className="project-department-tags">{projectDepartments.map(department=><span key={department}><Building2 size={12}/>{department}</span>)}{!projectDepartments.length&&<span>Departments not assigned</span>}</div></div></div>
    ...
    <div className="project-detail-grid">
      <section className="panel project-delivery-panel"><div className="panel-head"><span className="project-section-title"><span className="project-section-icon"><ClipboardList size={16}/></span><span><span className="panel-kicker">Current project work</span><span className="panel-title">Delivery tasks</span></span></span><span className="mono">{projectTasks.length} tasks</span></div><div className="list">{projectTasks.map(task=><button className="row project-task-row" key={task.id} onClick={()=>onOpen(task.id)}><div className="row-main"><div className="row-title">{task.title}</div><div className="row-meta">{task.code||task.id} · {taskDepartmentLabel(task)} · Due {task.due}</div></div><Status s={task.status}/><ChevronRight size={14}/></button>)}{!projectTasks.length&&<div className="empty"><strong>No delivery tasks yet</strong>Tasks connected to this project will appear here.</div>}</div></section>
      <aside className="project-side"><section className="panel detail-section"><div className="project-side-heading"><span className="project-section-icon people"><Users size={16}/></span><span><h3>Assigned people</h3><small>{currentAssigneeIds.length} on this project</small></span></div>{currentAssigneeIds.map(id=>{const person=team.find(item=>item.id===id);return <div className="project-person" key={id}><Avatar person={person}/><div><b>{person?.name||id}</b><div className="row-meta">{person?.position||employmentOf(person||emptyUser)} · {person?.department||"—"}</div></div></div>})}{!currentAssigneeIds.length&&<div className="row-meta">No people assigned.</div>}</section><section className="panel detail-section"><div className="panel-head"><span className="project-side-heading"><span className="project-section-icon resources"><FileText size={16}/></span><span><h3>Resources</h3><small>{project.resources.length} available</small></span></span>{isAdmin(user)&&<label className="btn"><Upload size={14}/>{uploading?"Uploading…":"Upload"}<input type="file" hidden disabled={uploading} accept="image/*,.pdf,.doc,.docx,.txt,.md" onChange={event=>{void addResource(event.target.files?.[0]);event.currentTarget.value=""}}/></label>}</div>{project.resources.length?project.resources.map(resource=><a className="resource-row" key={resource.id} href={resource.url||undefined} target={resource.url?"_blank":undefined} rel="noreferrer">{resource.kind==="image"?<Image size={14}/>:<FileText size={14}/>}<span>{resource.name}</span></a>):<div className="row-meta">No resources uploaded.</div>}</section></aside>
    </div>
  </div>;
}
  ...
  function TaskModal({onClose,onSave,user,team,projects}:{onClose:()=>void;onSave:(t:Task)=>Promise<void>;user:User;team:User[];projects:Project[]}){
    ...
    const toggleAssignee=(id:string,checked:boolean)=>{
      ...
      if(!checked&&taskLeadId===id)setTaskLeadId("");
       ...
       <div className="form-grid task-create-form" key={step}>
          {step===1?<>
...
function ObjectiveModal({objective,managers,projects,onClose,onSave}:{objective?:WeeklyObjective;managers:User[];projects:Project[];onClose:()=>void;onSave:(objective:Omit<WeeklyObjective,"id"|"createdBy"|"createdDate">)=>void}){
  ...
  const status=objective?.status||"Not started";
  return <Modal className="objective-modal" title={objective?"Edit weekly objective":"Create weekly objective"} onClose={onClose} footer={<><button className="btn" onClick={onClose}>Cancel</button><button className="btn primary" disabled={!title.trim()||!managerId||!dueDate||!projectIds.length} onClick={()=>onSave({title:title.trim(),description:description.trim(),managerId,projectIds,priority,dueDate,status})}>{objective?"Save changes":"Create objective"}</button></>}>
    <div className="form-grid">
