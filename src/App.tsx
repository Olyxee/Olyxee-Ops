...
const lastOnlineLabel=(value?:string|number)=>{
  if(!value)return"Never logged in";
  const timestamp=typeof value==="number"?value:new Date(value).getTime();
  if(!Number.isFinite(timestamp))return"Last login unavailable";
  ...
  if(days<7)return`Last online ${days}d ago`;
  return`Last online ${new Date(timestamp).toLocaleDateString([],{day:"numeric",month:"short",year:new Date(timestamp).getFullYear()===new Date().getFullYear()?undefined:"numeric"})}`;
};
...
function ProjectDetail({user,project,tasks,objectives,team,onBack,onOpen,onProjectUpdated,onResourceAdded,flash}:{user:User;project:Project;tasks:Task[];objectives:WeeklyObjective[];team:User[];onBack:()=>void;onOpen:(id:string)=>void;onProjectUpdated:(project:Project)=>void;onResourceAdded:(resource:ProjectResource)=>void;flash:(message:string)=>void}){
  ...
  const activeProjectTasks=projectTasks.filter(task=>["In Progress","Submitted for Review","Changes Requested"].includes(task.status)).length;
  const projectProgress=projectTasks.length?Math.round(completedProjectTasks/projectTasks.length*100):0;
  ...
  return <div className="project-detail project-detail-clear">
    ...
    <section className="project-summary-strip" aria-label="Project summary">
      <div className="project-summary-progress"><span className="project-summary-icon"><Check size={17}/></span><span><small>Overall progress</small><strong>{projectProgress}% complete</strong><i><b style={{width:`${projectProgress}%`}}/></i></span></div>
      <div><span className="project-summary-icon tasks"><ClipboardList size={17}/></span><span><small>Delivery tasks</small><strong>{completedProjectTasks} of {projectTasks.length} done</strong></span></div>
      <div><span className="project-summary-icon active"><Clock3 size={17}/></span><span><small>Work in motion</small><strong>{activeProjectTasks} active</strong></span></div>
      <div><span className="project-summary-icon people"><Users size={17}/></span><span><small>Project team</small><strong>{currentAssigneeIds.length} {currentAssigneeIds.length===1?"person":"people"}</strong></span></div>
    </section>
